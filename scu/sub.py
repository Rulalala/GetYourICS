    ...
    for x in data['xkxx']:
        for class1 in x.values():
            ...
            for i in timeList:
                ...
                ret_list = [coureName, teachername, '-'.join([campusName, teachingBuildingName, classroomName]), '-'.join(
                    [coureSequenceNumber, coureNumber]), weekDescription, classDay, time_to_list(str(classSessions)+'-'+str(int(continuingSession)+int(classSessions)-1))]
