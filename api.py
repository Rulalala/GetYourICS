...
def cquIcsGetIcsByUname(uname: str, upasswd: str) -> Response:
    try:
        if uname is None or upasswd is None:
            raise ValueError("用户名或密码格式不正确")
        s = cqu_ics.cquIcs(
...
@app.route('/api/v1/icsFile/<name>/<password>', subdomain="cqu", methods=['GET'])
def cquIcsApi(name,password):
    try:
        ret = None
        if request.method == "GET":
            if name is None or password is None:
                raise ValueError("用户名或密码格式不正确")
            # uname = request.args.get("uname",type=str, default=None)
            # pwd   = request.args.get("passwd",type=str, default=None)

            ret = cqu_ics.cquIcs(cqu_ics.getCquClassListByUname(name, password))
